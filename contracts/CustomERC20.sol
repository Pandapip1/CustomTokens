...
contract CustomERC20 is Multicall, Ownable, ERC2771Context {
    ...
    function _getTrueBalance(address holder) internal view initialized returns (uint256) {
        return _balances[holder] + _getTrueDistributionAmount(_privateDistributionAmount, _privateDistribution[holder]) - _balanceDebts[holder];
    }
    ...
    function _distributeTrueHolders(uint256 amount) internal initialized {
        _holderDistributionAmount += (_holderDistributionAmount + 10**18) * amount * _totalSupply - 10 ** 18;
        _totalSupply += amount;
    }
    ...
    function _subtractTrueBalance(address from, uint256 amount) internal initialized {
    ...
    function _transferTrue(address from, address to, uint256 amount) internal initialized {
        _subtractTrueBalance(from, amount);
        _addTrueBalance(to, _getTrueDistributionAmount(amount, _transferDistribution));
        _distributeTruePrivate(_getTrueDistributionAmount(amount, _totalPrivateDistribution));
        _distributeTrueHolders(_getTrueDistributionAmount(amount, _holderDistribution));
    }

    // Helpers
    function _trueToVisible(uint256 amount) internal view initialized returns (uint256) {
        return _getTrueDistributionAmount(amount, _holderDistributionAmount);
    }
    
    function _visibleToTrue(uint256 amount) internal view initialized returns (uint256) {
        // Can't be simplified :(
    ...
    function transfer(address to, uint256 value) public initialized returns (bool success) {
        _transferTrue(_msgSender(), to, _visibleToTrue(value));
        return true;
    }
    
    function transferFrom(address from, address to, uint256 value) public initialized returns (bool success) {
        require(_allowances[from][_msgSender()] >= value, "Not enough allowance");
    ...
    function approve(address spender, uint256 value) public initialized returns (bool success) {
        _allowances[_msgSender()][spender] = value;
        return true;
    }

    // Overrides
    function _msgData() internal override(Context, ERC2771Context) view returns (bytes memory) {
